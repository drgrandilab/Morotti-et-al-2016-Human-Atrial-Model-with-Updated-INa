function output = morotti_et_al_ham_ina_ran_model(t,y,p,runType)
...
% 63-100: INa Markov model (w/ ranolazine) - see lines 436-446
...
GNa = 10*(1-0.1*AF);  % [mS/uF]
GNaB = 1*0.597e-3;    % [mS/uF] 
IbarNaK = 1*1.26;     % [uA/uF]
...
I_Na_junc1 = Fjunc*GNa_hh*y(1)^3*y(2)*y(3)*(y(39)-ena_junc);
I_Na_sl1 = Fsl*GNa_hh*y(1)^3*y(2)*y(3)*(y(39)-ena_sl);
...
Q10_INa = 2.1; Tfactor_INa = Q10_INa^((Temp-300)/10);
...
I_Na_junc2 = Fjunc*GNa*(ONa+LONa)*(y(39)-ena_junc);
I_Na_sl2 = Fsl*GNa*(ONa+LONa)*(y(39)-ena_sl);
...
gkr = 0.035*sqrt(Ko/5.4)*factor_rano_kr;
...
gks_junc = (1+1*AF+2*ISO)*0.0035*1;
gks_sl = (1+1*AF+2*ISO)*0.0035*1; %FRA
...
% equations for activation;
Gkur = (1.0-0.5*AF)*(1+2*ISO)* 0.045*(1+0.2*RA); %nS/pF maleckar 0.045 
...
switch lower(protocol)
    ...
    case 'pace_cc', % current clamp (current injection at defined rate)
    ...
    case 'pace_cc_ead', % current clamp (20-s at 10-Hz, then 10-s at 1_Hz)
...
I_tot = I_Na_tot+I_Cl_tot+I_Ca_tot+I_K_tot; % [pA/pF]
ydot(39) = -(I_tot-I_app);
vmax = ydot(39);
